...
extension ProfViewController {
    fileprivate func preparemField() {
        ...
        let mob:String? = defaults.string(forKey: "mobile_no")
        print(mob as Any)
...
extension ProfViewController {
    ...
    func webSignUp(onCompletion: (() -> Void)? = nil, onError: ((Error?) -> Void)? = nil) {
        ...
        let pin = defaults.string(forKey: "pin") as String?
        print("Def:",mob ?? "N/A")
...
//        print("Mobile -> " + self.shareData.mobile)
        ...
        let req = """
        first_name=\(self.fField.text  ?? "N/A")&last_name=\(self.lField.text  ?? "N/A")&pin=\(pin  ?? "N/A")&dob=\(self.dField.text!)&gender=\(self.gender )&push_id=\("iOS_\(self.timestamp)")&mobile_no=\(mob ?? "N/A")&verified=\("1")&email=\(self.emailField.text  ?? "N/A")&login_type=\("Partner")
        """
